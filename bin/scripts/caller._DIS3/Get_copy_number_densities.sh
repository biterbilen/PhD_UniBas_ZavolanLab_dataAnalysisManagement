...
#TODO correct for this; in copies the density.. files are deleted but for T2C they are not
if [ -e density-$outf ]; then echo "$outf downdloaded from database"; exit; fi

#Get densities
odir=$chr$strand;
~bilebi00/GEORGES_POLYA/scripts/copy_allFeaturesToWiggle.pl $sampleId $chrid $strand $odir $annot $scalenorm "$excats" $minTagLen;
