...
#merge expression files into an expression matrix file
fcount=0;
for f in ${files[@]}; do 
	if [ "$f" == ${files[0]} ]; then
		cp $f $rand
	...
	echo $f
	~bilebi00/_PAPD5/scripts/outerJoin.pl $rand $f 1 1 '' 0 > $rand.tmp;
	i=$((i + $incr));
	fields="$fields,$i"
	mv $rand.tmp $rand
	fcount=$((fcount + 1));
...
#get expressed genes first from the datamatrix for raw and qn 
ext=.mclust
for f in $outf $outf.qn; do
	#TODO FIXME does not work; something weird with the numbers
	~bilebi00/bin/R --no-save --args $f $ext < ~bilebi00/_SHORT_READ_ALIGNMENT/scripts/classificationMixtureModel4matrix.R > /dev/null
done
