...
#c=""; s=_Get_xlinkEnrichment_pernuc.sh; for i in `seq 1 8`; do sed -e "s/_tagorder/$i/" $s > $s$i.$c; qsub $s$i.$c; echo $i; done
...
for s in "-" "+"; do
	...
	cat $indir/DB_all_$id/copies_$s | \
		...
		bedtools intersect -wao -s -b stdin -a $r.$s.gtf | sort -k1,9 | \
