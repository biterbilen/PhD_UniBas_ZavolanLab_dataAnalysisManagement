...
typenames=(mutation general annotation)
topNs=(100 100 100)
stacks=(0 0 0)
tag=EWSR1
tag=EWSR1_FlpIn
cell=HEK293
outdir=Analysis/Summary4Effective$cell
mkdir -p $outdir; pushd $outdir;

ids=(`less $sid_prot_f | grep -w -P "MCLIP|PARCLIP" | grep -w $cell | awk '$1==tag { print $2; }' tag=$tag | sort`)
...
for j in `seq 0 $((${#typenames[@]}-1))`; do
	...
	for i in `seq 0 $((${#ids[@]}-1))`; do
		...
		#get data from web08
		if [ "`echo $typename | grep mutation`" != "" ]; then
			ln -sf ~bilebi00/_CLIP/Analysis/UniqueRawData_ALL_woContamination/$id.${typename}_summary .
		else
			if [ ! -e $id.${typename}_summary ]; then 
				rsync -t mirz@bc2-web08.bc2.unibas.ch:~/BITER/UniqueRawData_ALL_woContamination/$id.${typename}_summary .
			else
				echo $id.${typename}_summary exists
			fi
		fi
		#prep for plotting
