...
k <- NULL
k$X1 <- d$X1
k["sum"] <- (d["445.annot"] + d["446.annot"]) / 2
#k["sum"] <- (d["445.all.annot"] + d["446.all.annot"]) / 2
#k["sum"] <- (d["288.shuffled.annot"] + d["289.shuffled.annot"]) / 2
mdn <- median(k$sum)
k <- data.frame(k)
...
barchart((value) ~ reorder(factor(X1), -sum), data=f, subset=sum>30, drop.unused.levels=T,
#	group=X2, ylab="Positions in top 1000 / All Positions",
	group=X2, ylab="Gene Category Enrichment",
