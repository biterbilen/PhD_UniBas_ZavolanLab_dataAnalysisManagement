...
cls=$inp.cls
#select top T2Cs
less $file | grep -P "$chr\s\\$str" | perl -e 'my $me=shift; my $dist=shift; my $t2c_cut=shift; my $lib_cut=shift; $ss="SS"; while(<>){chomp; (@t)=split; next if ($t[7] < $t2c_cut or $t[6] < $lib_cut); splice(@t,2,1); $d=0; $s=1; $s=-1 if($t[1] eq "-"); $t[2]+=$me; $t[3]-=$me; print join("\t","${ss}_0",@t,$d),"\n"; for (1..$dist) { $t[2]-=$s; $t[3]-=$s; print join("\t","SS_$_",@t,$d),"\n"; }}' $maxExt $dist $t2cCut $libCut > $inp
