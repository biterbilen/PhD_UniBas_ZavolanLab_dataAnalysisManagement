...
#Get densities
odir=$chr$strand;
~bilebi00/GEORGES_POLYA/scripts/copy_allFeaturesToWiggle.pl $sampleId $chrid $strand $odir mRNA $scaleNorm;
if [ -s $odir/copies ]; then
	#corrects for 0 index
	~bilebi00/GEORGES_POLYA/scripts/get_sameCopyNumberRegions_fromCopies.pl $odir/copies $strand > $odir/copies.refined
	~bilebi00/CDS_CLIP_ElMMo/scripts/add_geneSymbol.pl "^##" $odir/copies.refined ~bilebi00/CDS_CLIP_ElMMo/data/coordinates/EXON/ ~bilebi00/CDS_CLIP_ElMMo/data/hg_TR.info.all ~bilebi00/SRC/cluster_for_moreMM/generate_oriented_clusters_formutmodel $extension > $odir.copies.refined  
fi
