...
indir=~bilebi00/_CLIP/data/clipz_Aseq_sites/
project=ARE
...
#New
ids=(`less $sid_prot_f | grep -w $project | grep -w Aseq | cut -f 2`);
libs=(`less $sid_prot_f | grep -w $project | grep -w Aseq | cut -f 3`);
samplecount=${#ids[@]}
#

outdir=Analysis/Project_$project/PAchange_wIG/
...
if [ $task == 3 ] && [ $SGE_TASK_ID == 1 ]; then
	#TODO set using file called `fdr`
	...
	#3.55 0.100883962118176 24560 #batch1 of 303
	min_norm_tags=3.55
	...
	#exonsf=../annot_regions/ALL_hg19_UCSC_exon.andreasFormat $TODO genesf
	...
	#genesf=../annot_regions/ALL_hg19_UCSC_geneinfo.andreasFormat #TODO
