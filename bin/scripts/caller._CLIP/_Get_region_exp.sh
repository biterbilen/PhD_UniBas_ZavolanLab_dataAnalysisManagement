...
regionf=~bilebi00/_CLIP/Analysis/clipz_RNAseq/GeneExpression_GMExp_scaleNorm_Untreated/regions/hg19_GMAP_GENE${subregion}s_refseq.gtf.gz
geneStructuref=~bilebi00/_CLIP/Analysis/clipz_RNAseq/GeneExpression_GMExp_scaleNorm_Untreated/regions/hg19_GMAP_GENE_refseq.gtf.gz

outdir=Analysis/clipz_RNAseq/GeneExpression_GMExp_scaleNorm_Untreated/RegionExpression_$subregion
...
if [ $SGE_TASK_ID -le ${#ids[@]} ]; then
	...
	#use reads that are spanned by a region
...
if [ $SGE_TASK_ID == 1 ]; then
	...
	#merge
	statf=all.stats
	echo -e "tag\ttype\tmu\tsd\tN\tid" > $statf
	cat freqstat* >> $statf
