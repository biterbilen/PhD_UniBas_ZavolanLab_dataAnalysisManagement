...
#$ -cwd
#$ -t 1-16
#$ -o LOG._Get_xlinkEnrichment_summary4_cov.sh.$TASK_ID
...
#-----------------
posteriors=(1000 800 400 200 100 50 40 20)
jobcount=${#posteriors[@]}
...
#-----------------
outdir=Analysis/xlinkEnrichment/basedOnNucleotide_${annot}_${c}/$tag/coverage/summary
...
if [ $posteriori == 0 ]; then
	for i in `seq 1 ${#ifs[@]}`; do
		ifile=${ifs[$(($i-1))]}
		ln -sf $ifile .
		less $ifile | awk '$5>posterior && $5 != "NA" {print}' posterior=${posteriors[$((${#posteriors[@]}-1))]} | sort -k 5,5gr | gzip -c > buffer.$i.bed.gz
	done
...
if [ $posteriori == 0 ]; then
	...
	for i in `seq 1 ${#ofs[@]}`; do
		...
		less $ofile | bedtools cluster -s -d $((extension-1)) -i stdin | sort -k 1,1 -k 7,7 -k 5,5gr | \
			bedtools groupby -i stdin -g 1,7 -c 5 -o min -full | cut -f 1-6 | sort -k 5,5gr > repr_$ofile
	done
