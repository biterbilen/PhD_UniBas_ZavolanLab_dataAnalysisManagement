...
i1s=(3 4 6 8)
i2s=(2 3 5 7)
clipfiles="../../xlinkEnrichment/annotation/DIS3L2/pernuc_merged_XL.gene_mRNA"; tag1=DIS3L2
clipfiles2="../../xlinkEnrichment/annotation/AGO2/pernuc_merged_XL.gene_mRNA"; tag2=AGO2
...
#---
#fake ChIP intersection
cat $clipfiles2 |  sed 's/^ *//g' | sed 's/";*//g' | sed 's/ /\t/g' | sort -k 2,2 | \
	bedtools groupby -i stdin -g 2 -c 1 -o sum | awk '{print $2"\t"$1;}' > chip
echo `head -n 1 $file.w_clip` AGO2xlinkCount AGO2xlinkStatus | sed 's/ /\t/g' > $file.w_clip.w_chip
~bilebi00/_PAPD5/scripts/leftJoin.pl $file.w_clip chip $gi 2 '' 0 | \
	awk 'BEGIN{OFS="\t";}{if($(NF-1)>0){$NF="CLIPPED";} else{$NF="NOTCLIPPED";} if(NR>1){print}}' >> $file.w_clip.w_chip
...
for i in `seq 0 $((${#i1s[@]}-1))`; do
	...
	tag=`less $file.w_clip.w_chip | head -n 1 | cut -f $i1`.`less $file.w_clip.w_chip | head -n 1 | cut -f $i2`
	echo $tag
	...
	less $r$i | awk 'BEGIN{OFS="\t";} { if (NR>1){ if ($5=="CLIPPED")  { if ($6=="CLIPPED") { $5="BOTH"; } else { $5=tag1; } } else { if ($6=="CLIPPED") { $5=tag2; } else { $5=tag3;  }}  print; }}' tag1=${tag1}_CLIPPED tag2=${tag2}_CLIPPED tag3=NONE | \
	 	sort -k 5,5 | \
...
~bilebi00/bin/R --no-save --args log2Ratio_clip_chip.stats "Expression Fold Change (log2)" "$tag1 and $tag2 CLIP Statistics" "" 1 < ~bilebi00/_EWSR1/scripts/CI.R > /dev/null
~bilebi00/bin/R --no-save --args log2Ratio_clip.stats "Expression Fold Change (log2)" "DIS3L2 CLIP Statistics" "" 1 < ~bilebi00/_EWSR1/scripts/CI.R > /dev/null
~bilebi00/bin/R --no-save --args log2Ratio_chip.stats "Expression Fold Change (log2)" "AGO2 CLIP Statistics" "" 1 < ~bilebi00/_EWSR1/scripts/CI.R > /dev/null
